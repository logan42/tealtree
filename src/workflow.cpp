...
void Workflow::init_registries()
{
    ...
    Feature::registry.register_class<DenseFeatureImpl<16>>();
        ...
        if (!sparse_v1) {
            logger->info("Using sparse features V2.");
        }
...
#ifndef NDEBUG
    logger->warn("TealTree is compiled with debug checks. Expect lower performance.");
#endif
    ...
    for (size_t i = 0; i < data->documents.size(); i++) {
        ...
        if (max_score > 1e12) {
            if (!this->msg_score_too_large) {
                logger->warn("Document scores are getting too large. For ranker this might indicate overfitting.");
...
void Workflow::log_gradient()
{
    ...
    logger->trace("Computed gradients: {}.", oss.str());
    if (this->options.step == Step::newton) {
        ...
        logger->trace("Computed hessians: {}.", oss.str());
    }
}
...
void Workflow::train_a_tree(uint32_t tree_index)
{
    ...
    if (root->split->spread > 0) {
        ...
        logger->warn("Warning: cannot split root in this tree. This might indicate overfitting.");
    ...
    while (data->current_tree->get_nodes().size() < n_tree_nodes) {
        if (heap.empty()) {
            if (!this->msg_tree_too_short) {
                ...
                logger->warn("Terminating tree before max leaves reached.");
            }
            break;
        }
        ...
        logger->trace("Node #{} has {} docs, splitting by feature {} inverse={} bucket={}",
            best_split->node->node_id, best_split->node->doc_ids.size(), 
            best_split->feature->get_name(), best_split->inverse, best_split->threshold);
        ...
        logger->trace("Left Node #{} has {} docs, Right Node #{} has {}.",
            children.first->node_id, children.first->doc_ids.size(),
            children.second->node_id, children.second->doc_ids.size());
    ...
    logger->info("Tree #{} trained in {} seconds.",
        tree_index, format_float(TIMER_FINISH(t), 3));
...
std::unique_ptr<DataFileReader> Workflow::get_tsv_reader(std::shared_ptr<ColumnConsumerProvider> ccp, bool with_query)
{
    ...
    logger->info("Reading data from {} in {} format{} ...",
        input_source, to_string(this->options.input_format), sample_rate_clause);
    return result;
}
...
std::tuple<const std::vector<float_t>, const std::vector<DOC_ID>, Workflow::FEATURE_PIPELINE_PTR_TYPE  > Workflow::read_tsv()
{
    ...
    logger->info(
        ...
        ccp_ptr->label_consumer->get_data()->size(), 
        ccp_ptr->query_consumer->get_query_limits()->size() - 1,
        ccp_ptr->features.size());
    logger->info("Cooking features...");
...
void Workflow::log_feature_types(std::map<std::string, size_t> & feature_types, char prefix, std::string msg_prefix)
{
    ...
    logger->info("{} {}.", msg_prefix, oss.str());
}
